...
# Spawn the mobs
execute as @s at @s run summon parched ~ ~ ~ {equipment:{mainhand:{id:"minecraft:bow",count:1}}}

# Add new mob tag, and any subsequent NBT changes
execute as @s at @s run tag @e[type=minecraft:parched,distance=..2,tag=!gz-a4-oldmob] add gz-a4-newmob
