...
class EventActions(MCActionsBase):
    ...
    def wait_for_right_block_hit(self, player_name: 'str') -> 'Vec3':
        target_name = self.mcplayer.name if (not player_name or player_name == 'SELF') else player_name
        q = queue.Queue()
        self.mcplayer.mj.events.subscribe_local('rightBlockHit', q)
        try:
            while True:
                if self.mcplayer.cancel_event and self.mcplayer.cancel_event.is_set():
                    raise PowerCancelledException
                try:
                    event_obj = q.get(timeout=0.1)
                    if target_name == 'ALL' or getattr(event_obj, 'name', None) == target_name:
                        return event_obj.pos.clone()
                except queue.Empty:
                    continue
        finally:
            self.mcplayer.mj.events.unsubscribe_local('rightBlockHit', q)

    @mced_block(
        label="Wait for left block hit by a Player",
        player_name={'label': 'Player name', 'shadow': '<shadow type="text"><field name="TEXT">SELF</field></shadow>'}
    )
    def wait_for_left_block_hit(self, player_name: 'str') -> 'Vec3':
        target_name = self.mcplayer.name if (not player_name or player_name == 'SELF') else player_name
        q = queue.Queue()
    ...
    def wait_for_block_place(self, player_name: 'str') -> 'Vec3':
        target_name = self.mcplayer.name if (not player_name or player_name == 'SELF') else player_name
        q = queue.Queue()
        self.mcplayer.mj.events.subscribe_local('blockPlace', q)
        try:
            while True:
                if self.mcplayer.cancel_event and self.mcplayer.cancel_event.is_set():
                    raise PowerCancelledException
            ...
            self.mcplayer.mj.events.unsubscribe_local('blockPlace', q)

    @mced_block(
        label="Wait for block break by a Player",
        player_name={'label': 'Player name', 'shadow': '<shadow type="text"><field name="TEXT">SELF</field></shadow>'}
    )
    def wait_for_block_break(self, player_name: 'str') -> 'Vec3':
        target_name = self.mcplayer.name if (not player_name or player_name == 'SELF') else player_name
        q = queue.Queue()
        self.mcplayer.mj.events.subscribe_local('blockBreak', q)
        try:
            while True:
                if self.mcplayer.cancel_event and self.mcplayer.cancel_event.is_set():
                    raise PowerCancelledException
            ...
            self.mcplayer.mj.events.unsubscribe_local('blockBreak', q)

    @mced_block(
        label="Wait for chat message from a Player",
        player_name={'label': 'Player name', 'shadow': '<shadow type="text"><field name="TEXT">SELF</field></shadow>'}
    )
    def wait_for_chat(self, player_name: 'str') -> 'str':
        target_name = self.mcplayer.name if (not player_name or player_name == 'SELF') else player_name
        q = queue.Queue()
        self.mcplayer.mj.events.subscribe_local('chat', q)
        try:
            while True:
                if self.mcplayer.cancel_event and self.mcplayer.cancel_event.is_set():
                    raise PowerCancelledException
                try:
                    event_obj = q.get(timeout=0.1)
                    if target_name == 'ALL' or getattr(event_obj, 'name', None) == target_name:
                        return event_obj.message
                except queue.Empty:
                    continue
        finally:
            self.mcplayer.mj.events.unsubscribe_local('chat', q)

    @mced_block(
        label="Wait for projectile to hit a block"
    )
    def wait_for_projectile_hit_block(self) -> 'Vec3':
        q = queue.Queue()
        self.mcplayer.mj.events.subscribe_local('projectileHitBlock', q)
        try:
            while True:
                if self.mcplayer.cancel_event and self.mcplayer.cancel_event.is_set():
                    raise PowerCancelledException
            ...
            self.mcplayer.mj.events.unsubscribe_local('projectileHitBlock', q)

    @mced_block(
        label="Wait for projectile to hit an entity"
    )
    def wait_for_projectile_hit_entity(self) -> 'str':
        q = queue.Queue()
        self.mcplayer.mj.events.subscribe_local('projectileHitEntity', q)
        try:
            while True:
                if self.mcplayer.cancel_event and self.mcplayer.cancel_event.is_set():
                    raise PowerCancelledException
                try:
                    event_obj = q.get(timeout=0.1)
                    return event_obj.targetName
                except queue.Empty:
                    continue
        finally:
            self.mcplayer.mj.events.unsubscribe_local('projectileHitEntity', q)

    @mced_block(
        label="Wait for projectile launch by a Player",
        player_name={'label': 'Player name', 'shadow': '<shadow type="text"><field name="TEXT">SELF</field></shadow>'}
    )
    def wait_for_projectile_launch(self, player_name: 'str') -> 'str':
        target_name = self.mcplayer.name if (not player_name or player_name == 'SELF') else player_name
        q = queue.Queue()
        self.mcplayer.mj.events.subscribe_local('projectileLaunch', q)
        try:
            while True:
                if self.mcplayer.cancel_event and self.mcplayer.cancel_event.is_set():
                    raise PowerCancelledException
                try:
                    event_obj = q.get(timeout=0.1)
                    if target_name == 'ALL' or getattr(event_obj, 'shooterName', None) == target_name:
                        return event_obj.projectileType
                except queue.Empty:
                    continue
        finally:
            self.mcplayer.mj.events.unsubscribe_local('projectileLaunch', q)
    ...
    def wait_for_player_respawn(self, player_name: 'str') -> 'Vec3':
        target_name = self.mcplayer.name if (not player_name or player_name == 'SELF') else player_name
        q = queue.Queue()
        self.mcplayer.mj.events.subscribe_local('playerRespawn', q)
        try:
            while True:
                if self.mcplayer.cancel_event and self.mcplayer.cancel_event.is_set():
                    raise PowerCancelledException
            ...
            self.mcplayer.mj.events.unsubscribe_local('playerRespawn', q)