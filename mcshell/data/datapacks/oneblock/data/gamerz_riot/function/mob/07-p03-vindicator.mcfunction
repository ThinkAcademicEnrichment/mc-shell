...
# Spawn the mobs
execute as @s at @s run summon minecraft:vindicator ~ ~1.6 ~ {equipment:{mainhand:{id:"minecraft:iron_axe",count:1,components:{"minecraft:enchantments":{"minecraft:knockback":1}}}}}
execute as @s[scores={gz-a4-mobamount=2..}] at @s run summon minecraft:vindicator ~ ~1.6 ~ {equipment:{mainhand:{id:"minecraft:iron_axe",count:1,components:{"minecraft:enchantments":{"minecraft:knockback":1}}}}}

# Add new mob tag, and any subsequent NBT changes
execute as @s at @s run tag @e[type=minecraft:vindicator,distance=..2,tag=!gz-a4-oldmob] add gz-a4-newmob
