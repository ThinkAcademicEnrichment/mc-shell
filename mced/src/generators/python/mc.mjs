...
export function defineMineCraftGenerators(pythonGenerator) {
     ...
     */
        ...
        // Hook into the generator's init method to reset state before each run.
        // We handle the potential previous init method by storing it in a variable.
    const previousInit = pythonGenerator.init;

    pythonGenerator.init = function (workspace) {
        // Call the previous init function if it existed
        if (previousInit) {
    ...
    /**
     * The 'finish' method is called after all blocks are processed.
     * It wraps the generated code in the class structure.
     */
    pythonGenerator.finish = function (code) {
        // Standard imports needed by the runner
        ...
        // The header defines the class and the constructor
        // active_threads list is initialized here to track background tasks
        const header = `
...
`;
        ...
        // Inject user-defined methods (Powers) into the class
        const definitions = Object.values(pythonGenerator.methods_ || {});
        const classIndent = pythonGenerator.INDENT; // Standard indentation (4 spaces usually)

        // Prefix each method line with class indentation so it sits inside BlocklyProgramRunner
        ...
        // Define run_program method
        const runMethodHeader = `
    def run_program(self):
        if self.cancel_event and self.cancel_event.is_set(): return
`;

        // Indent user code (the workspace blocks) to fit inside run_program
        // It needs two levels of indentation: Class -> Method -> Code
        const methodBodyIndent = classIndent + classIndent;

        // If 'code' is empty (empty workspace), use 'pass'
        const indentedBlockCode = pythonGenerator.prefixLines(code, methodBodyIndent) || (methodBodyIndent + 'pass');

        // Conditional Footer:
        // If threads were detected, we inject the wait loop.
        let footer = "";

        if (pythonGenerator.hasThreads) {
            // We generate the wait loop code and indent it to match the method body
            const rawFooter = `
...
try:
    while True:
        ...
        # Filter list to only alive threads
    ...
    pass
`;
            footer = '\n' + pythonGenerator.prefixLines(rawFooter, methodBodyIndent);
        } else {
            footer = ""; // Finite execution, just return naturally
        }

        // Combine: Imports + Class Header + User Methods + run_program Header + Block Code + Footer Loop
    ...
    pythonGenerator.forBlock['procedures_callreturn'] = function (block, generator) {
        ...
        for (let i = 0; i < argNames.length; i++) {
            const varName = generator.nameDB_.getName(argNames[i], MCED.BlocklyNameTypes.VARIABLE);
            const connectedBlock = block.getInputTargetBlock('ARG' + i);
            let argCode = connectedBlock ? (generator.valueToCode(block, 'ARG' + i, generator.ORDER_NONE) || 'None') : `self.runtime_params.get('${varName}', None)`;
            args.push(`${varName}=${argCode}`);
        }
        const code = 'self.' + funcName + '(' + args.join(', ') + ')';
